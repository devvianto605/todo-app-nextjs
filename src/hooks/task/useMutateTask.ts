...
const handleAddTask = useMutation({
    mutationFn: addTask,
    onSuccess: async () => await queryClient.invalidateQueries({queryKey: [QUERY_KEY.GET_TASK]})
})

const handleEditTask = useMutation({
    mutationFn: ({body, taskId}:{body: EditTaskRequest; taskId: string}) => editTask({body, taskId}),
    onSuccess: async () => await queryClient.invalidateQueries({queryKey: [QUERY_KEY.GET_TASK]})
})
